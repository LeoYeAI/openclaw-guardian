/**
 * buddy-claw — Virtual Pet Companion Plugin for OpenClaw
 *
...
const buddyPlugin = {
  ...
  register(api: any) {
    const config = resolveConfig(api.config?.get?.('buddy-claw'));
    if (!config.enabled) {
      api.logger.info('[buddy-claw] Plugin disabled by config');
      return;
    }

    const store = new BuddyStore(config.dbPath);
    api.logger.info(
      `[buddy-claw] Plugin loaded (reactChance=${config.reactChance})`,
    );
    ...
    if (typeof api.registerCleanup === 'function') {
      api.registerCleanup(() => {
        store.close();
        api.logger.info('[buddy-claw] Plugin unloaded, DB closed');
      });
    }
  },
};
